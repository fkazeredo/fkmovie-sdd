...
public class PricingAdminService {
    ...
    public List<PricingSeatType> listSeatTypes() {
        return seatTypes.findAll();
    }

    @Transactional(readOnly = true)
    public List<PricingWeekday> listWeekdays() {
        return weekdays.findAll();
    }

    /** Updates seat-type surcharges (SPEC-0012); ACCESSIBLE/COMPANION must stay 0. */
    @Transactional
    public List<PricingSeatType> updateSeatTypes(Map<SeatType, Integer> surcharges, UUID actingAdminId) {
    ...
    @Transactional
    public List<PricingWeekday> updateWeekdays(Map<Integer, BigDecimal> multipliers, UUID actingAdminId) {
