...
public class ScreeningService {
    ...
    @Transactional
    public Screening create(UUID movieId, UUID roomId, Instant startsAt, int basePriceCents, UUID actingAdminId) {
        var now = Instant.now();
    ...
    public Page<Screening> list(UUID roomId, UUID movieId, Instant from, Instant to, Pageable pageable) {
        return screenings.search(roomId, movieId, from, to, pageable);
    }

    @Transactional(readOnly = true)
    public Screening get(UUID id) {
    ...
    @Transactional
    public Screening cancel(UUID id, UUID actingAdminId) {
        var screening = get(id);
